...
- 新增强制生产门：主美冻结批次任务包和关键技术约束，3D原画以单方案批量方式制作整批轴测图并直接交用户验收；用户筛选三视图对象，主美补齐动态／关键装配技术合同后，再进行三视图或简化尺寸说明验收及逐资产 Blender／AI 3D／混合路线审批；完成前禁止建模。
...
- **2026-09-02 覆盖规则**：对可动或关键装配对象，`b06-parallel-program-p0010-p0014-p0016-procedural-prototype-pipeline` 的已验收基础几何功能原型与版本化 `FunctionalRigContract` 是建模的额外前置门。模型只能替换合同声明的视觉槽；本条覆盖本 change 中任何可被解读为“先建模、后由程序适配”的顺序表述，不改变既有视觉或失败证据。
...
- 主要影响独立美术项目 `D:\unity\UnityProject\ArtResource` 中的前置概念图、技术设计文档、Blender／Unity 美术源、模块库、LookDev／样机场景、材质、贴图、Prefab、配置与验收证据。
