...
- [x] 4.1 实现不可变 `PersistentId`、`0` 无效规则、比较/哈希/格式化和统一 `ulong` 高水位分配器（P0-004）
- [x] 4.2 实现新分配、既有 ID 恢复、高水位推进、溢出拒绝和已注销 ID 永不复用，并建立 EditMode 测试（P0-004）
- [x] 4.3 实现带对象类别验证的注册、注销和查询结果，拒绝无效 ID、重复 ID、类别不匹配和静默覆盖（P0-004）
- [x] 4.4 实现只保存 ID 与预期类别的持久引用语义，测试目标缺失、重新载入同一 ID、同名对象不自动重绑及跨会话隔离（P0-004）
