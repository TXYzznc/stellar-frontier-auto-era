...
- 为现有 GameData 生成器增加领域无关的项目输出配置，使自动纪元生成代码进入产品目录和命名空间；贯通最小 DataTable、Config、Language 加载及错误诊断。
...
- 产品资源：新增主菜单与第一版空世界场景，以及项目 GameData 最小贯通输入和生成物。
- 框架接入：更新 `AppConfigs` 的 Procedure、DataTable、Config、Language 登记。
- 通用工具：DataTable 生成器只增加领域无关的可配置输出根和命名空间能力，不包含自动纪元业务判断；若触及 `ScriptsBuiltin`，必须通过框架纯度与回归测试。
