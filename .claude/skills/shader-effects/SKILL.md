---
name: shader-effects
description: 视觉着色器效果——发光/泛光、色差、扭曲、暗角、胶片颗粒、扫描线、故障效果、溶解、描边和菲涅尔效果。适用于为着色器添加视觉润色、后期处理效果或风格化渲染的场景。
tags: shader-effects, post-processing, glsl-programming, visual-rendering, webgl-visuals
tags_cn: Shader视觉效果, 后期处理, GLSL编程, 视觉渲染, WebGL视觉特效
---

# Shader效果

为着色器输出添加润色、风格化效果与氛围的视觉特效集合。
...
- `references/effect-order.md` — 效果组合的推荐顺序
- `references/performance.md` — 各效果的性能开销
