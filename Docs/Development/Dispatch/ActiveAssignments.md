...
|---|---|---|---|---|---|---|---|
| `b02-parallel-program-p0003-p0006-runtime-foundations` | `P0-003`～`P0-006` | 程序（客户端）；测试后续验收 | 实施中 | — | 既有精确写锁继续保留；后续段使用`GameData/AIData/{DataTables,Configs,Languages}/Foundation/`、Editor-only Profile及用户已精确授权的框架文件 | 主工程8090仍由客户端保留，未进PlayMode | 第3段已提交`03c71e9`并通过12/12 EditMode测试；继续下一实施段 |
...
| Git索引／提交 | 程序（客户端）／`b02-parallel-program-p0003-p0006-runtime-foundations` | 第4段一次性短锁 | 2026-08-24 | 仅限已回传的6个显式文件；提交完成或失败后立即释放并回传结果 |
