...
{
    /// <summary>
    /// OperationFeedbackForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/OperationFeedbackForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 OperationFeedbackForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    /// 规格页序切换、取消意图与默认焦点。Grp_PageHost 下的内容页顺序即规格页序。
    /// </summary>
    public sealed partial class OperationFeedbackForm : AutoEraShellFormBase
    {
