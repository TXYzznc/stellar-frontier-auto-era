...
{
    /// <summary>
    /// QuestForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/QuestForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 QuestForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    /// 规格页序切换、取消意图与默认焦点。Grp_PageHost 下的内容页顺序即规格页序。
    /// </summary>
    public sealed partial class QuestForm : AutoEraShellFormBase
    {
        /// <summary>本界面所属领域尚未接入运行路径；原因写在每一页的空态里。</summary>
        public const string NotWiredReason =
            "任务域尚未接入运行路径：任务状态与领取结算都还没有创建者。";
        ...
        /// </summary>
        private static readonly int[] NavigationPageIndex = { 0, 0, 0 };
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _questsLoadingState, _questsEmptyState, _questsErrorState, _questsSuccessState, _questsDisabledState, _questsListBody, _questsDetailBody);
            ShowPageUnavailable(NotWiredReason, _questRewardsLoadingState, _questRewardsEmptyState, _questRewardsErrorState, _questRewardsSuccessState, _questRewardsDisabledState, _questRewardsRewardsBody, _questRewardsUnlocksBody);
