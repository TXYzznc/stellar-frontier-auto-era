...
{
    /// <summary>
    /// WarehouseForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/WarehouseForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 WarehouseForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    ...
    public sealed partial class WarehouseForm : AutoEraShellFormBase
    {
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _inventoryLoadingState, _inventoryEmptyState, _inventoryErrorState, _inventorySuccessState, _inventoryDisabledState, _inventoryCatalogBody, _inventoryDetailBody);
            ShowPageUnavailable(NotWiredReason, _itemDetailLoadingState, _itemDetailEmptyState, _itemDetailErrorState, _itemDetailSuccessState, _itemDetailDisabledState, _itemDetailItemBody, _itemDetailStockBody);
            ShowPageUnavailable(NotWiredReason, _warehouseRecordsLoadingState, _warehouseRecordsEmptyState, _warehouseRecordsErrorState, _warehouseRecordsSuccessState, _warehouseRecordsDisabledState, _warehouseRecordsEventsBody, _warehouseRecordsDetailBody);
