...
{
    /// <summary>
    /// RuleHelpForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/RuleHelpForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 RuleHelpForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    /// 规格页序切换、取消意图与默认焦点。Grp_PageHost 下的内容页顺序即规格页序。
    /// </summary>
    public sealed partial class RuleHelpForm : AutoEraShellFormBase
    {
        /// <summary>本界面所属领域尚未接入运行路径；原因写在每一页的空态里。</summary>
        public const string NotWiredReason =
            "通用规则说明尚未接入运行路径：规则条目还没有录入本地化表。";
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _ruleHelpLoadingState, _ruleHelpEmptyState, _ruleHelpErrorState, _ruleHelpSuccessState, _ruleHelpDisabledState, _ruleHelpDescriptionBody);
