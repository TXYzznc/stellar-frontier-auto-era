...
{
    /// <summary>
    /// SettingsForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/SettingsForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 SettingsForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    ...
    public sealed partial class SettingsForm : AutoEraShellFormBase
    {
        ...
        public const string NotWiredReason =
            "用户设置域尚未接入运行路径：声音、操作与显示设置还没有持久化载体。";
        ...
        /// </summary>
        private static readonly int[] NavigationPageIndex = { 0, 1, 2 };
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _audioSettingsLoadingState, _audioSettingsEmptyState, _audioSettingsErrorState, _audioSettingsSuccessState, _audioSettingsDisabledState, _audioSettingsVolumesBody, _audioSettingsPersistenceBody);
            ShowPageUnavailable(NotWiredReason, _controlSettingsLoadingState, _controlSettingsEmptyState, _controlSettingsErrorState, _controlSettingsSuccessState, _controlSettingsDisabledState, _controlSettingsCameraBody, _controlSettingsBindingsBody);
            ShowPageUnavailable(NotWiredReason, _displaySettingsLoadingState, _displaySettingsEmptyState, _displaySettingsErrorState, _displaySettingsSuccessState, _displaySettingsDisabledState, _displaySettingsDisplayBody, _displaySettingsQualityBody);
