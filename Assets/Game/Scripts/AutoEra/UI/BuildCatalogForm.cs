...
{
    /// <summary>
    /// BuildCatalogForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/BuildCatalogForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 BuildCatalogForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    /// 规格页序切换、取消意图与默认焦点。Grp_PageHost 下的内容页顺序即规格页序。
    /// </summary>
    public sealed partial class BuildCatalogForm : AutoEraShellFormBase
    {
        /// <summary>本界面所属领域尚未接入运行路径；原因写在每一页的空态里。</summary>
        public const string NotWiredReason =
            "建造图纸域尚未接入运行路径：图纸目录与解锁状态没有数据来源。";
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _blueprintsLoadingState, _blueprintsEmptyState, _blueprintsErrorState, _blueprintsSuccessState, _blueprintsDisabledState, _blueprintsCatalogBody, _blueprintsBlueprintBody);
