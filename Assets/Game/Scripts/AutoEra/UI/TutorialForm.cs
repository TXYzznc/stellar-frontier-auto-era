...
{
    /// <summary>
    /// TutorialForm 的 GF 桥。结构由 Docs/Development/UI-PrefabLayouts/TutorialForm.contract.json 生成
    /// （设计来源：Docs/GameDesign/03-玩家体验/界面规格）。
    ///
    /// 绑定字段在同名的 TutorialForm.Fields.cs 里（同一 partial 类）；本文件只有类逻辑：
    /// 规格页序切换、取消意图与默认焦点。Grp_PageHost 下的内容页顺序即规格页序。
    /// </summary>
    public sealed partial class TutorialForm : AutoEraShellFormBase
    {
        /// <summary>本界面所属领域尚未接入运行路径；原因写在每一页的空态里。</summary>
        public const string NotWiredReason =
            "教程内容尚未接入运行路径：步骤与触发条件还没有配置载体。";
        ...
        protected override void OnAutoEraOpen()
        {
            ...
            ShowPageUnavailable(NotWiredReason, _tutorialLoadingState, _tutorialEmptyState, _tutorialErrorState, _tutorialSuccessState, _tutorialDisabledState, _tutorialLessonBody, _tutorialLinksBody);
