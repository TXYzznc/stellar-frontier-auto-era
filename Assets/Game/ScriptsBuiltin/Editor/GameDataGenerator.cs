        ...
        public static void RefreshAllDataTable(IList<string> fullPathFiles = null)
        {
            try
            {
                ...
                for (int i = 0; i < totalExcelCount; i++)
                {
                    ...
                    {
                        Debug.LogErrorFormat("Excel -> DataTable 失败。文件: '{0}'。异常: {1}", excelFileName, e);
                        break;
                    }
                }
                ...
                for (int i = 0; i < dataTbCount; i++)
                {
                    ...
                    {
                        Debug.LogErrorFormat("生成 DataTable 代码失败。表: '{0}'，文件: '{1}'。异常: {2}", dataTableName, tbTxtFile, e);
                    }
                }
            ...
            {
                Debug.LogErrorFormat("刷新 DataTable 时发生未处理异常: {0}", e);
            }
            finally
            {
                EditorUtility.ClearProgressBar();
