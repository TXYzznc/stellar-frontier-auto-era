...
public class GFBuiltin : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            GFTrace.BeginTrace("runtime");
            GFTrace.Info("GFBuiltin", "Awake.Begin");
            Instance = this;
            var resCom = GameEntry.GetComponent<ResourceComponent>();
            if (resCom != null)
            {
                var resTp = resCom.GetType();
                var mResourceMode = resTp.GetField("m_ResourceMode", BindingFlags.Instance | BindingFlags.NonPublic);
                mResourceMode.SetValue(resCom, AppSettings.Instance.ResourceMode);
                GFBuiltin.Log($"------------Set ResourceMode:{AppSettings.Instance.ResourceMode}------------");
                GFTrace.Success("GFBuiltin", "ResourceMode.Set", null, GFTrace.Data("resourceMode", AppSettings.Instance.ResourceMode.ToString()));
            }
        }
    }
    ...
    public void UpdateCanvasScaler()
    {
        ...
        GFBuiltin.Log($"----------UI Match:{canvasScaler.matchWidthOrHeight}----------");
    }
